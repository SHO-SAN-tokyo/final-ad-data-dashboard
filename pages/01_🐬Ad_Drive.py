...
df_num = df_num.rename(columns={**rename_common, "コンバージョン数": "conv_total"})
df_banner = df_banner.rename(columns={**rename_common, "CV": "conv_banner"})
...
# --- 2段目: メインカテゴリ・サブカテゴリ・広告媒体・広告目的・特殊カテゴリ ---
...
with apply_col1:
    applied = st.button("✅ この条件で絞り込む", type="primary", key="apply_filters")
with apply_col2:
    reset = st.button("♻ 条件をクリア", key="reset_filters")

if "ad_filters_applied" not in st.session_state:
    st.session_state["ad_filters_applied"] = False
if applied:
    st.session_state["ad_filters_applied"] = True
if reset:
    st.session_state["ad_filters_applied"] = False

# ──────────────────────────────────────────────
# ④ フィルター関数（キャンペーン / バナー共通）
# ──────────────────────────────────────────────
def apply_filters(
    df: pd.DataFrame,
...
# ④〜⑧ は適用ボタンが押された時だけ実行
# ──────────────────────────────────────────────
if st.session_state["ad_filters_applied"]:
    ...
    # バナーは画像 URL がある行だけ軽量表示 (最大 100 件)
    df_banner_disp = (
        df_banner_filt[df_banner_filt["CloudStorageUrl"].notna()].head(100)
    ...
    # ──────────────────────────────────────────
    # ⑥-A キャンペーン一覧表 UI（広告セット名・キーワードは未適用）
    # ──────────────────────────────────────────
    ...
    st.info("🔎 条件を設定して「✅ この条件で絞り込む」を押すと、集計結果が表示されます。")
