...
if isinstance(sel,(list,tuple)) and len(sel)==2:
    s,e = map(pd.to_datetime, sel); df = df[(df["Date"].dt.date>=s.date())&(df["Date"].dt.date<=e.date())]
else:
    d = pd.to_datetime(sel).date(); df = df[df["Date"].dt.date==d]

st.sidebar.header("🔍 フィルター")
for col,lbl in [("PromotionName","クライアント"),("カテゴリ","カテゴリ"),("CampaignName","キャンペーン名")]:
    opts = ["すべて"]+sorted(df[col].dropna().unique())
    sel  = st.sidebar.selectbox(lbl, opts)
    if sel!="すべて": df = df[df[col]==sel]

for i in range(1,61):
    c=str(i); df[c]=pd.to_numeric(df.get(c,0), errors="coerce").fillna(0)
...
conv_series = pd.to_numeric(df["コンバージョン数"], errors="coerce").fillna(0)   # ← 修正
tot_conv = conv_series.sum()
...
st.subheader("📊 集計サマリー"); st.table(summary)
...
latest = (img.dropna(subset=["Date"]).sort_values("Date")
         .loc[lambda d:d.groupby(["CampaignId","AdName"])["Date"].idxmax()].copy())
...
agg=(df.assign(AdName=lambda d:d["AdName"].astype(str).str.strip(),
      ...
      .reset_index())
latest=latest.merge(agg[["CampaignId","AdName","Cost"]],on=["CampaignId","AdName"],
...
for i,(_,r) in enumerate(latest.iterrows()):
    key=(r["CampaignId"],r["AdName"]); s=sum_map.get(key,{})
    cost,imp,clk=s.get("Cost",0),s.get("Impressions",0),s.get("Clicks",0)
    cv=int(r["CV件数"]); cpa=div(cost,cv); ctr=div(clk,imp)
    text=r.get("Description1ByAdType","")

    links=urls(r.get("canvaURL",""))
