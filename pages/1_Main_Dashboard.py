...
sd = st.sidebar.date_input("日付フィルター", (min_d, max_d), min_d, max_d)
if isinstance(sd, (list, tuple)) and len(sd) == 2:
    s, e = map(pd.to_datetime, sd)
...
st.sidebar.text_input("クライアント検索", "", key="client_search", on_change=update_client)

search_val = st.session_state.get("client_search", "")
show_clients = [c for c in clients if search_val.lower() in c.lower()] if search_val else clients
client_opts = ["すべて"] + show_clients
sel_client = st.sidebar.selectbox(
    "クライアント", client_opts, index=client_opts.index(st.session_state.get("selected_client", "すべて"))
)
if sel_client != "すべて":
...
img_df["CV件数"] = img_df.apply(get_cv, axis=1).fillna(0).astype(int)
...
for i, (_, r) in enumerate(img_df.iterrows()):
    ...
    """
    card = f"<div class='ad-card'><img src='{r['CloudStorageUrl']}'>{caption}</div>"
    with cols[i % 5]:
