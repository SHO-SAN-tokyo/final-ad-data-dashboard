...
try:
    df = bq.query("SELECT COUNT(*) as cnt FROM `shosan-ad-expertai.SHOSAN_Notion_Data.NOTION_JOINED_AD_DATA`").to_dataframe()
    st.success(f"✅ BQアクセスOK: {df['cnt'][0]} 件")
except Exception as e:
    st.error(f"❌ BQアクセスエラー: {e}")
