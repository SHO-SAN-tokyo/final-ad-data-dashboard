...
def normalize_blanks(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ✅ start_month / end_month だけ：YYYY-MM 以外は全部 NULL 扱い
    for c in ["start_month", "end_month"]:
        if c in df.columns:
            s = df[c]
            df[c] = s.where(s.isna() | s.str.match(MONTH_RE), pd.NA)
...
# ✅ 表示時だけ：NULL は空欄表示にして "None" を見せない（保存時に正規化するので安全）
display_df = current_df.copy()
for c in ["担当者", "所属", "雇用形態", "operator_id", "start_month", "end_month"]:
    if c in display_df.columns:
        display_df[c] = display_df[c].astype("string").fillna("")
        display_df[c] = display_df[c].replace({"None": "", "none": "", "nan": "", "NaN": ""})

editable_df = st.data_editor(
    display_df.sort_values(["担当者", "start_month"]),
